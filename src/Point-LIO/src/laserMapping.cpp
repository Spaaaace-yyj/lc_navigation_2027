...
inline rclcpp::Time sec2time(double sec) {
...
void publish_init_map(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubLaserCloudFullRes)
{
    ...
    laserCloudmsg.header.stamp = sec2time(lidar_end_time);
...
void publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMapped)
{
    ...
    if (publish_odometry_without_downsample) {
        odomAftMapped.header.stamp = sec2time(time_current);
    } else {
        odomAftMapped.header.stamp = sec2time(lidar_end_time);
    }
    ...
    if (!br) br = std::make_shared<tf2_ros::TransformBroadcaster>(node_ptr);
...
void publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pubPath)
{
    ...
    msg_body_pose.header.stamp = sec2time(lidar_end_time);
...
int main(int argc, char** argv)
{
    ...
    node_ptr = std::make_shared<rclcpp::Node>("laserMapping");
    readParameters(node_ptr);
    cout<<"lidar_type: "<<lidar_type<<endl;
    ivox_ = std::make_shared<IVoxType>(ivox_options_);
    
    path.header.stamp    = sec2time(lidar_end_time);
...
if (p_pre->lidar_type == AVIA) {
    sub_pcl_custom = node_ptr->create_subscription<livox_ros_driver2::msg::CustomMsg>(
        lid_topic, rclcpp::SensorDataQoS(),
        [](const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr msg) { livox_pcl_cbk(msg); });
} else {
    sub_pcl_standard = node_ptr->create_subscription<sensor_msgs::msg::PointCloud2>(
        lid_topic, rclcpp::SensorDataQoS(),
        [](const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg) { standard_pcl_cbk(msg); });
}
auto sub_imu = node_ptr->create_subscription<sensor_msgs::msg::Imu>(
    imu_topic, rclcpp::SensorDataQoS(),
    [](const sensor_msgs::msg::Imu::ConstSharedPtr msg) { imu_cbk(msg); });

auto pubLaserCloudFullRes      = node_ptr->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered", 1000);
auto pubLaserCloudFullRes_body = node_ptr->create_publisher<sensor_msgs::msg::PointCloud2>("/cloud_registered_body", 1000);
auto pubLaserCloudMap          = node_ptr->create_publisher<sensor_msgs::msg::PointCloud2>("/Laser_map", 1000);
auto pubOdomAftMapped          = node_ptr->create_publisher<nav_msgs::msg::Odometry>("/aft_mapped_to_init", 1000);
auto pubPath                   = node_ptr->create_publisher<nav_msgs::msg::Path>("/path", 1000);
//------------------------------------------------------------------------------------------------------
